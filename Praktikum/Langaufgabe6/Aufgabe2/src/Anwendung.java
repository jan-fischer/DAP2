...
public class Anwendung {
	...
	public static int[] latenessScheduling(ArrayList<Job> jobs) {
		...
		for (int i = 0; i < n; i++) {
			//da die Tupel nach Deadline aufsteigend sortiert sind ,wählen wir die Erste 
			//und erhöhen die Summe(Z) um Jobdauer
			
			A[i] = z;
			z += jobs.get(i).getDauer();
			int verspaetung = z-jobs.get(i).getDeadline(); // Berechnung der Verspätung
			if(verspaetung > maxVerspaetung) maxVerspaetung = verspaetung; // ggf. erhohen wenn größer als Maximal 
		}
		
		
		System.out.println("Versp�tung: "+maxVerspaetung); // Drucken maxverspätung aus
		return A;
	}
